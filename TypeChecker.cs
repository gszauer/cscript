    ...
    class TypeChecker : Pass0.StatementVisitor<Pass1.Statement, TypeEnvironment>, Pass0.ExpressionVisitor<Pass1.Expression, TypeEnvironment> {
            ...
            foreach (Pass0.Statement s in program) {
                if (s is Pass0.VarDeclStatement) {
                    ...
                    if (varDecl.Initializer != null) {
                        if (!(varDecl.Initializer is Pass0.LiteralExpression)) {
                            throw new CompilerException(ExceptionSource.TYPECHECKER, varDecl.Location, "Global variables can only be initialized to literals");
                        }
        ...
        public Pass1.Expression VisitCallExpression(Pass0.CallExpression expr, TypeEnvironment misc) {
            ...
            if (!(calee is Pass1.VariableExpression)) {
                throw new InterpreterException(calee.Location, "Only variable expression is callable, not: " + calee.Type.DebugName);
            }
            ...
            if (!mTypeTable.IsCallable(calee.Type)) {
                throw new InterpreterException(calee.Location, "Calling function on non callable type: " + calee.Type.DebugName);
            }
            ...
            if (callable.ArgumentNames.Count != args.Count) {
                throw new InterpreterException(calee.Location, "Calling " + varExpr.Name + " with wrong number of arguments: " + args.Count + " / " + callable.ArgumentNames.Count);
            }

            for (int i = 0; i < args.Count; ++i) {
                if (callable.ArgumentTypes[i] != args[i].Type) {
                    throw new InterpreterException(calee.Location, "Calling " + varExpr.Name + " with argument (" + (i + 1) + ") '" + callable.ArgumentNames[i] + "' being the wrong type, expected: " + callable.ArgumentTypes[i].DebugName + ", got: " + args[i].Type.DebugName);
                }
            }
